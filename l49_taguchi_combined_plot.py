...
def create_combined_mean_effect_plot(data, results_dir):
    """创建四个因子的组合均值效应图"""
    ...
    fig.suptitle('L49田口实验参数均值效应图', fontsize=16, fontweight='bold')
    ...
    for factor_id, factor_data in factor_effects.items():
        ...
        # 绘制均值效应图 - 使用与独立图一致的样式
        ax.plot(levels, means, 'bo-', linewidth=2, markersize=6, markerfacecolor='blue')
        ...
        # 设置标题和标签 - 与独立图一致
        ax.set_title(factor_names[factor_id], fontsize=14, fontweight='bold')
        ax.set_xlabel('水平值', fontsize=12)
        if col == 0:  # 只在左侧子图显示y轴标签
    ...
    # 保存图片
    output_path = Path(results_dir) / 'L49_田口实验组合均值效应图.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
    plt.close()
    
    print(f"✅ 组合均值效应图已保存: {output_path}")
...
def main():
    ...
    try:
        ...
        # 创建组合均值效应图
        create_combined_mean_effect_plot(data, results_dir)
